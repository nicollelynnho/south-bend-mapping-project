...
facilities <- read_csv('Public_Facilities.csv')
...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel("Parks, Facilities, and Amenities",
             fluidPage(
               fluidRow(
                 column(width = 3,
                 ...
                 column(width = 9,
                        ...
                        fluidRow(
                          column(width = 12,
                                 h6("Facilities Frequency"),
                                 plotOutput("facilityFreq", height = 300)
                          )
                        )
                 )
               )
...
server <- function(input, output, session) {
  ...
  #facilities geo
  facilities_geo <- reactive({
    dat <- facilities
    
    if (!is.null(input$zipGeo) && length(input$zipGeo) > 0) {
      dat <- dat %>%
        filter(as.character(POPL_ZIP) %in% as.character(input$zipGeo))
    }
...
# Setting up color palette that sensitive to color blindness
  ...
  #facility
  output$facilityFreq <- renderPlot({
    
    df <- facilities_geo() %>%
      filter(!is.na(POPL_TYPE), POPL_TYPE != "") %>%
      count(POPL_TYPE, name = "Count")
    
    req(nrow(df) > 0)
    
    ggplot(df,
           aes(x = reorder(POPL_TYPE, Count), y = Count)) +
      geom_col(fill = "#FF7F00") +
      coord_flip() +
      theme_minimal() +
      labs(
        title = "Facility Frequency by Population Type",
        x = "Facility Type",
  ...
  output$abandon_density_chart <- renderPlotly({
    ...
    arrange(desc(n)) %>%
      mutate(
        SchoolAbbrev = factor(SchoolAbbrev, levels = SchoolAbbrev)
      ) %>%
      plot_ly(
        x = ~SchoolAbbrev,
        y = ~n,
